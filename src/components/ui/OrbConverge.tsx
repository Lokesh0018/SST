...
const PERIOD = 4.8 // seconds for one loop at Speed 50
...
// A raised cosine over a clamped input: 0 at both ends, 1 in the middle, and
// FLAT at both ends, so a loop driven by it holds at each extreme without that
// hold being a separate state.
...
// Cartesian to [polar angle from +y, azimuth]. Loops that move a dot ALONG the
// surface work in these, because moving the angle keeps the dot on the sphere
// where moving the point would not.
...
// thirty-seven; this is the only part that is "converge".
function frame(t: number, P: Params, out: Dot[]) {
        const n = dotsN(150, P.n)
        // Raised cosine: flat at both ends, so the ring holds and the sphere
        // holds without either needing to be its own state.
        const k = bump(t)
        for (let i = 0; i < n; i += 1) {
            const p = polar(fib(i, n))
            // Interpolate the polar ANGLE toward the equator. Lerping the point
            // instead would deflate the ball into a disc on the way.
...
function __OriginkitBase_OrbConverge(props: Props) {
    ...
    vRef.current = {
        dot: dotColor,
        // This loop never asks for the accent ink, so there is no Accent control
        // to feed it -- a row that maps to nothing is worse than a missing one.
        acc: dotColor,
